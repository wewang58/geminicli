...
1.  **Build a specific PR**:
    ```bash
    /build-pr --pr 1234 --all
    ```

2.  **Build Hypershift CLI from a PR**:
    ```bash
    /build-pr --pr 1234 --cli
    ```

3.  **Build HCP CLI from a PR**:
    ```bash
    /build-pr --pr 1234 --hcp
    ```

4.  **Build Control Plane Operator (CPO) image from a PR**:
    ```bash
    /build-pr --pr 1234 --cpo-image
    ```
...
5.  **Build and push images to a repository**:
    ...
    ```
6.  **Build CLI and operator image**:
    ```bash
    /build-pr --cli --operator-image
    ```

7.  **Build everything with a specific tag**:
    ```bash
    /build-pr --all --tag my-feature
    ```

8.  **Build with custom image names**:
    ```bash
