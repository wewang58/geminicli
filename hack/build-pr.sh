...
usage() {
    ...
    echo "  --pr <id>          Checkout a specific PR before building"
...
if [ -n "$PR_ID" ]; then
    info "Checking out PR #$PR_ID..."
    if ! git diff-index --quiet HEAD --; then
        echo "Error: Repository is not clean. Please commit or stash your changes."
        exit 1
    fi
    git fetch upstream "pull/$PR_ID/head:pr-$PR_ID" || git fetch origin "pull/$PR_ID/head:pr-$PR_ID"
